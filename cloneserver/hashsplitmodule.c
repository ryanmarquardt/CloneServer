...
inline uint chunk_digest(Chunk *self) { return (self->s1 << 16) | self->s2; }
...
void chunk_append(Chunk *self, unsigned char add) {
	const int cursor = self->length - MIN_CHUNK_SIZE;
...
PyMODINIT_FUNC init_hashsplit(void) {
	...
	PyModule_AddObject(mod, "DEFAULT_MAX_CHUNK_SIZE", PyInt_FromLong(DEFAULT_MAX_CHUNK_SIZE));
}
